...
@section('title', trans('cms::cms.pages'))
@section('content_header', trans('cms::cms.pages'))
@section('breadcrumb')
    <ol class="breadcrumb">
        <li class="active">@lang('cms::cms.pages')</li>
    </ol>
@endsection
                    ...
                    <tbody>
                        @forelse ($posts as $i => $post)
                            <tr>
                                ...
                                <td>
                                    @foreach (config('app.languages') as $languageCode => $languageName)
                                        @if ($post->hasTranslation($languageCode))
                                            <a href="{{ route('backend.pages.edit', [$post->id] + ['locale' => $languageCode]) }}">
                                ...
                                <td align="center">
                                    <a class="btn btn-default btn-xs" href="{{ route('backend.pages.edit', [$post->id] + request()->query()) }}"><i class="fa fa-pencil"></i></a>
                                    <a class="btn btn-danger btn-xs" href="{{ route('backend.pages.trash', $post->id) }}" onclick="return confirm('@lang('cms::cms.are_you_sure_to_delete_this')?')"><i class="fa fa-trash-o"></i></a>
                                    @can('backend pages delete')
                                        <a class="btn btn-danger btn-xs" href="{{ route('backend.pages.delete', $post->id) }}" onclick="return confirm('@lang('cms::cms.are_you_sure_to_delete_this_permanently')?')"><i class="fa fa-trash-o"></i></a>
                                    @endcan
                                </td>
                            </tr>
                        @empty

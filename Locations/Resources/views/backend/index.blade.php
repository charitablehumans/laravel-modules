...
@section('title', trans('cms::cms.locations'))
@section('content_header', trans('cms::cms.locations'))
@section('breadcrumb')
    <ol class="breadcrumb">
        <li class="active">@lang('cms::cms.locations')</li>
    </ol>
@endsection
...
@section('content')
    <div class="box">
        <div class="box-header with-border">
            <a class="btn btn-default btn-sm" href="{{ route('backend.locations.create', request()->query()) }}">@lang('cms::cms.create')</a>
        </div>
        <div class="box-body table-responsive">
            <form action="{{ route('backend.locations.index') }}" method="get">
                    ...
                    <thead>
                        ...
                        <tr>
                            ...
                            <th>
                                ...
                                <a class="btn btn-default btn-xs" href="{{ route('backend.locations.index') }}"><i class="fa fa-repeat"></i></a>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($posts as $i => $post)
                            <tr>
                                ...
                                <td>
                                    @foreach (config('app.languages') as $languageCode => $languageName)
                                        @if ($post->hasTranslation($languageCode))
                                            <a href="{{ route('backend.locations.edit', [$post->id] + ['locale' => $languageCode]) }}">
                                ...
                                <td align="center">
                                    <a class="btn btn-default btn-xs" href="{{ route('backend.locations.edit', [$post->id] + request()->query()) }}"><i class="fa fa-pencil"></i></a>
                                    <a class="btn btn-danger btn-xs" href="{{ route('backend.locations.trash', $post->id) }}" onclick="return confirm('@lang('cms::cms.are_you_sure_to_delete_this')?')"><i class="fa fa-trash-o"></i></a>
                                    @can('backend locations delete')
                                        <a class="btn btn-danger btn-xs" href="{{ route('backend.locations.delete', $post->id) }}" onclick="return confirm('@lang('cms::cms.are_you_sure_to_delete_this_permanently')?')"><i class="fa fa-trash-o"></i></a>
                                    @endcan
                                </td>
                            </tr>
                        @empty
